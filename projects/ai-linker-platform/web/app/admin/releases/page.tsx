...
import { Upload, Download, Globe, EyeOff, Star } from 'lucide-react'

export default async function ReleasesPage() {
  const releases = await prisma.agentRelease.findMany({
    orderBy: { createdAt: 'desc' },
    include: {
      agentProduct: { select: { name: true, slug: true } },
      installerFile: true,
    },
  })
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">AI Agent 인스톨러 빌드 {releases.length}개 — DB 실데이터</p>
        </div>
        <Button size="sm" className="gap-1.5" disabled title="Gate 4에서 업로드 연결">
              ...
              {releases.map((release) => (
                <tr key={release.id} className="border-b border-border/50 hover:bg-muted/30 transition-colors">
                  <td className="px-4 py-3">
                    <p className="font-medium">{release.agentProduct.name}</p>
                    <p className="font-mono text-muted-foreground mt-0.5">{release.agentProduct.slug}</p>
                  </td>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center justify-center gap-1">
                      <button className="p-1.5 rounded hover:bg-muted transition-colors text-muted-foreground hover:text-foreground" title="다운로드 테스트"><Download className="w-3.5 h-3.5" /></button>
                      <button className="p-1.5 rounded hover:bg-muted transition-colors text-muted-foreground hover:text-foreground" title={release.status === 'PUBLISHED' ? '비공개' : '공개'}>{release.status === 'PUBLISHED' ? <EyeOff className="w-3.5 h-3.5" /> : <Globe className="w-3.5 h-3.5" />}</button>
                    </div>
                  </td>
                </tr>
              ))}
