...
export default async function TokensPage({ searchParams }: TokensPageProps) {
  const session = await requireAdminPagePermission('TOKENS_READ')

  const { q = '' } = await searchParams
  const query = q.trim()
  const currentMonthStart = monthStart()

  const [wallets, transactions, totalBalance, monthlyCharges, lowBalanceCount, usersWithoutWallet, activeCustomers] = await Promise.all([
    ...
    prisma.user.findMany({
      ...
      take: 200,
      orderBy: { createdAt: 'desc' },
      select: { id: true, name: true, email: true, creditWallet: { select: { balanceUsd: true } } },
    }),
  ])

  const canManageTokens = hasAdminPermission(session, 'TOKENS_MANAGE')
  const creditTargets = activeCustomers.map((customer) => ({
    userId: customer.id,
    name: customer.name,
    email: customer.email,
    balanceUsd: customer.creditWallet?.balanceUsd ?? 0,
  }))
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        ...
        {canManageTokens ? (
          <CreditAdjustmentButton customers={creditTargets} />
        ) : (
          <div className="rounded-md border border-dashed border-border px-3 py-2 text-xs text-muted-foreground">
            크레딧 지급/차감은 슈퍼관리자 전용입니다.
          </div>
        )}
      </div>
              ...
              <table className="w-full text-xs">
                <thead>
                  <tr className="border-b border-border bg-muted/40">
                    ...
                    {canManageTokens && <th className="text-center px-4 py-3 font-medium text-muted-foreground">액션</th>}
                  </tr>
                </thead>
                <tbody>
                  {wallets.map((wallet) => {
                  ...
                  {wallets.length === 0 && (
                    <tr>
                      <td colSpan={canManageTokens ? 6 : 5} className="px-4 py-10 text-center text-sm text-muted-foreground">조건에 맞는 크레딧 지갑이 없습니다.</td>
                    </tr>
                  )}
                </tbody>
              </table>
